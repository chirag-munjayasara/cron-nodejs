...
app.post(routes.save, function(req, res) {
	// new job
	ERRORMAIL="m.chirag@valuebound.com";
	// console.log(req.body.command);
	var file_path = "/var/www/html/cron-job-files/";
	var site_name = req.body.job_name;
	var site_env = req.body.env;
	var ext = ".sh";
	var file_name = file_path + site_name + "-" + site_env + ext;
	var error_file_name = file_path + site_name + "-" + site_env + ".log";
	console.log(file_name);
	console.log(error_file_name);
	let SUBJECT="ERROR in "+ site_name + " at " + site_env + " Environment";

	let command = "#!/bin/bash";
	command += "\n";
	command += "PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin";
	command += "\n";
	command += "FILENAME="+error_file_name;
	command += "\n";
	command += "SUBJETC="+SUBJECT;
	command += "\n";
	command += "terminus drush "+ site_name + "." + site_env + " -- cron > $FILENAME";
	command += "\n";
	command += "STATUS=`grep 'error' $FILENAME`"
	command += "\n";
	command += 'if [ -z "$STATUS" ]'
	command += "\n";
	command += "then";
	...
	command += "else"
	command += "\n";
	command += "mail -s $SUBJECT $ERROR_MAIL < $FILENAME"
	command += "echo some cron fails";
	command += "\n";
	command += "fi";
	console.log(command);
	fs.writeFile(file_name, command, (err) => {
		if (err) throw err;
